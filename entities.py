...
def animation_sequence(sequence,base_path,colorkey=(255,0,255)):
    global animation_database
    result = []
...
class animation(object):
    ...
    def __init__(self,sequence,base_path,attributes=[],colorkey=(255,0,255)):
        self.sequence = animation_sequence(sequence,base_path,colorkey)
    ...
    def play(self,key,surf,show=True): # play both displays and handles the frame
        global animation_database
        anim_read = self.active_animations[key]
        if show == True:
            if anim_read[3] == None:
                surf.blit(animation_database[self.sequence[anim_read[2]]],(anim_read[0],anim_read[1]))
            else:
                surf.blit(anim_read[3],(anim_read[0],anim_read[1]))
